...
def train_and_attack_with_defense(args, X_a_train, X_b_train, y_train):
    ...
    if valid_inferences.sum().item() > 0:
        ...
        print_attack_metrics(attack_metrics, 'Direct_LIA_w_BWL_Defense', args.dataset)
    else:
        # 如果没有有效推断，所有指标都为0
...
if __name__ == '__main__':
    ...
    vfl_models, attack_metrics = train_and_attack_with_defense(args, X_a_train, X_b_train, y_train)

    # 保存攻击评估结果（不再测试主任务准确率）
    save_attack_results('Direct_LIA_w_BWL_Defense', args.dataset, attack_metrics)
